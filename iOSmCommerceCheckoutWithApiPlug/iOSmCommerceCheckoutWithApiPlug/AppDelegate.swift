//
//  AppDelegate.swift
//  iOSmCommerceCheckoutWithApiPlug
//
//  Created by Rahul Panzade on 20/03/19.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        if GeneralClass.isConnectedToNetwork(){
            if GetApiConfig.execute(){
                ...
                if (UserDefaults.standard.value(forKey: "StartURLFromServer") as! NSString?) == nil{
                    showErrorAlert(message: AlertTitleNMessage.SOMETHING_WRONG_MESSAGE.rawValue)
                }else{
                    self.loadInitialViewController()
                }
            }
            ...
            if (UserDefaults.standard.value(forKey: "StartURLFromServer") as! NSString?) == nil{
                self.showErrorAlert(message: AlertTitleNMessage.NOINTERNET_MESSAGE.rawValue)
    ...
    private func loadInitialViewController(){
        ...
        let initialViewController : UINavigationController = mainStoryboard.instantiateViewController(withIdentifier: "UINavigationController") as! UINavigationController
